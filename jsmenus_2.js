/*
 * The only difference between this version and jsmenus.js is that the set to display: none occurs AFTER outfunc is called, NOT before (to allow effects to control hiding and showing menus.
 *
 */
...
function close_menu(butid,menuid,outfunc){
	...
	if(but.getAttribute('on') != 'true' && menu.getAttribute('on') != 'true'){
		...
		if(outfunc === undefined) outfunc = function (butid, menuid) { document.getElementById(menuid).style.cssText = 'display: none;';}
		outfunc(butid,menuid);
	}
}
